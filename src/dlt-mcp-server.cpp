...
DltMcpServer::DltMcpServer() {
    settings_ = std::make_unique<QSettings>("MyCompany", "DLTViewer_DltMcpServer");
    dashboard_ = nullptr;
    dlt_file_ = nullptr;
    initMcpServer();
    registerMcpTools();
...
QString DltMcpServer::pluginInterfaceVersion() { return PLUGIN_INTERFACE_VERSION; }

QString DltMcpServer::description() { return "MCP Server for DLT log analysis"; }
...
void DltMcpServer::selectedIdxMsgDecoded(int index, QDltMsg& /*msg*/) { selected_index_ = index; }
...
bool DltMcpServer::decodeMsg(QDltMsg& /*msg*/, int /*triggeredByUser*/) { return false; }
...
void DltMcpServer::indexMessage(int index, const QDltMsg& msg) {
    ...
    index_.insert({index, timestamp, ecu_time, getCtidIndex(ctid), getApidIndex(apid),
                   getEcuidIndex(ecuid), level});
}
...
char DltMcpServer::getLevelChar(int level) {
    ...
    if (it != log_levels_.end()) {
        return static_cast<char>(std::toupper(static_cast<unsigned char>(it->second[0])));
    }
    return '?';
}
...
std::tuple<int64_t, int64_t, int64_t, int64_t> DltMcpServer::splitRelativeTime(int64_t offsetNs) {
    int64_t total_ms = offsetNs / 1'000'000;
    return {total_ms / 3600000, (total_ms % 3600000) / 60000, (total_ms % 60000) / 1000,
            total_ms % 1000};
}

std::tuple<int64_t, int64_t> DltMcpServer::splitRelativeTimestamp(int64_t offsetNs) {
...
std::string DltMcpServer::formatMessageLine(int64_t hours, int64_t minutes, int64_t seconds,
    ...
    return fmt::format("+{:02}:{:02}:{:02}.{:03} {} {}/{} #{} @{}.{:06} T{}.{:03} ", hours, minutes,
                       seconds, millis, levelChar, ctid, apid, msgIndex, relSec, relUsec, ecuSec,
                       ecuMmm);
}
...
void DltMcpServer::registerMcpTools() {
    mcp::tool summary_tool =
        mcp::tool_builder("get_log_summary")
            .with_description("Get statistics and summary info of the loaded DLT log, such as "
                              "range of timestamps and list of available CTID/APID pairs")
            .build();
    server_->register_tool(summary_tool, [this](const auto& params, const auto& session_id) {
        return get_log_summary(params, session_id);
    });
    mcp::tool search_tool =
        mcp::tool_builder("search")
            .with_description(
                "Retrieve previews of log lines, optionally filtered by CTID, APID, log level(s), "
                "timestamp range, keyword and paginated by limit/offset. Log lines include "
                "relative time offset (+HH:MM:SS.mmm), log level, CTID/APID, message ID (#), "
                "relative timestamp in seconds from log start (@), and ECU uptime in seconds (T). "
                "The value after @ can be reused directly as min_timestamp or max_timestamp "
                "parameter (float64, seconds from start of log). If payload exceeds 100 "
                "characters, the log line is a preview and ends with symbol ~. Use count=true to "
                "get only the number of matching lines. Use case_insensitive=true for "
            ...
            .with_number_param("min_log_level", "Minimal log level to include into output", false)
            .with_number_param("max_log_level", "Maximal log level to include into output", false)
            .with_number_param("min_timestamp", "Starting timestamp in seconds from log start",
                               false)
            .with_number_param("max_timestamp", "Ending timestamp in seconds from log start", false)
            .with_string_param("keyword",
                               "Substring to search in the message payload (regular expressions or "
                               "wildcards are not supported)",
                               false)
            .with_boolean_param(
                "case_insensitive",
                "When true, perform case-insensitive keyword matching. Default: false.", false)
            .with_boolean_param("count",
                                "When true, return only the total number of matching lines instead "
                                "of the lines themselves. Ignores limit/offset.",
                                false)
            .with_number_param("limit", "Limit output to this many log lines, hard limit is 100",
                               false)
            .with_number_param("offset",
                               "Offset of the result from the start of selected log range", false)
            .build();
    server_->register_tool(search_tool, [this](const auto& params, const auto& session_id) {
        return search(params, session_id);
    });
    mcp::tool retrieve_tool =
        mcp::tool_builder("get_messages")
            .with_description("Retrieve a list of log messages with given IDs (#), including full "
                              "payload. The limit is 20 messages per tool call.")
            .with_array_param("message_ids", "IDs of the log messages to retrieve", "number")
            .build();
    server_->register_tool(retrieve_tool, [this](const auto& params, const auto& session_id) {
        return get_messages(params, session_id);
    });
    mcp::tool selection_tool =
        mcp::tool_builder("get_selection")
            .with_description("Get details of the currently selected message in the DLT Viewer. "
                              "Returns metadata including index, CTID/APID, log level, timestamps, "
                              "and a payload preview. Returns an error if no message is selected.")
            .build();
...
mcp::json DltMcpServer::get_log_summary(const mcp::json& /*params*/,
    ...
    if (!dlt_file_) {
        throw mcp::mcp_exception(mcp::error_code::internal_error, "DLT file is not loaded");
    }
    if (dlt_file_->size() == 0) {
        throw mcp::mcp_exception(mcp::error_code::internal_error, "DLT file is empty");
    }
    ...
    std::string log_start =
        fmt::format("{}.{:06}", first_msg.getGmTimeWithOffsetString(0, false).toStdString(),
...
mcp::json DltMcpServer::search(const mcp::json& params, const std::string& /*session_id*/) {
    if (!dlt_file_) {
        throw mcp::mcp_exception(mcp::error_code::internal_error, "DLT file is not loaded");
    }
    if (dlt_file_->size() == 0) {
    ...
    double min_timestamp_sec = has_min_ts ? params["min_timestamp"].get<double>() : 0;
    double max_timestamp_sec = has_max_ts ? params["max_timestamp"].get<double>() : 0;
    ...
    bool case_insensitive =
        params.contains("case_insensitive") ? params["case_insensitive"].get<bool>() : false;
    ...
    int64_t base_ts = getBaseTimestamp();
    auto& index = index_.get<0>();
    int64_t min_ts_ns = base_ts + static_cast<int64_t>(min_timestamp_sec * 1'000'000'000.0);
    int64_t max_ts_ns = base_ts + static_cast<int64_t>(max_timestamp_sec * 1'000'000'000.0);
    ...
    for (auto it = it_lower; it != it_upper && static_cast<int>(matches.size()) < max_collect;
         ++it) {
        const auto& entry = *it;
        ...
        if (has_keyword) {
            QDltMsg msg;
            if (!dlt_file_->getMsg(entry.index, msg))
                continue;
            QString payload = msg.toStringPayload();
            Qt::CaseSensitivity cs = case_insensitive ? Qt::CaseInsensitive : Qt::CaseSensitive;
            if (payload.indexOf(keyword, cs) == -1)
                continue;
        }

        matches.push_back(
            {entry.index, entry.timestamp, entry.ecu_time, entry.ctid, entry.apid, entry.level});
    }
    ...
    for (int i = offset; i < static_cast<int>(matches.size()); i++) {
        ...
        // Fetch full payload.
        QDltMsg msg;
        if (!dlt_file_->getMsg(m.msg_index, msg))
            continue;
        QString payload = msg.toStringPayload();
        ...
        oss << formatMessageLine(hours, minutes, seconds, millis, getLevelChar(m.level),
                                 ctids_[m.ctid], apids_[m.apid], m.msg_index, rel_sec, rel_usec,
                                 ecu_sec, ecu_mmm);
...
mcp::json DltMcpServer::get_messages(const mcp::json& params, const std::string& /*session_id*/) {
    if (!dlt_file_) {
        throw mcp::mcp_exception(mcp::error_code::internal_error, "DLT file is not loaded");
    }
    ...
    for (const auto& id_json : ids) {
        ...
        oss << formatMessageLine(hours, minutes, seconds, millis, getLevelChar(level), ctid_str,
                                 apid_str, msg_index, rel_sec, rel_usec, ecu_sec, ecu_mmm);
...
mcp::json DltMcpServer::get_selection(const mcp::json& /*params*/,
    ...
    std::ostringstream oss;
    oss << formatMessageLine(hours, minutes, seconds, millis, getLevelChar(msg.getSubtype()),
                             msg.getCtid().toStdString(), msg.getApid().toStdString(),
